...
- **`preload.cjs` is hand-copied, not compiled.** [env-scripts/start-electron-dev.cjs](env-scripts/start-electron-dev.cjs) copies it into `dist-electron/electron/` on each start, [env-scripts/build-and-start-electron-prod.cjs](env-scripts/build-and-start-electron-prod.cjs) does the same, and `build:electron` copies it too so `build:prod` cannot package a stale one. Editing it mid-session does nothing until you restart the dev script.
...
- **The user-facing app name is "Storybook Packager" in exactly two places, and they must agree.** `build.productName` in [package.json](package.json) names the installer, install directory, and shortcuts; `app.setName('Storybook Packager')` at the top of [src/electron/main.ts](src/electron/main.ts) names the userData folder and the macOS app menu. The kebab-case `storybook-packager` that remains in the npm `name`, the `appId`, the GitHub repo, and the update-check User-Agent is identifier-shaped and stays lowercase — do not "unify" those. Changing `app.setName` moves `app.getPath('userData')`, orphaning recents, window state, legal acceptance, and update state, so it needs a migration once there are real users; [env-scripts/start-electron-dev.cjs](env-scripts/start-electron-dev.cjs) hardcodes the same name in `getUserDataDir()` and must be changed in lockstep.
...
- Help → About sends `menu:help-about` to the focused window, which opens [src/components/AboutModal.tsx](src/components/AboutModal.tsx). The welcome window has no menu bar, so it triggers the same modal from an About button. Check for Updates lives both inside that modal and in the Help menu.
- **Updates are notify-only and there is no auto-updater.** The app is unsigned on every platform, so `electron-updater` is deliberately not a dependency. [src/electron/updateCheck.ts](src/electron/updateCheck.ts) reads `releases/latest` from GitHub's public API for the `GITHUB_OWNER`/`GITHUB_REPO` constants defined there (`Lin87/storybook-packager` — intentionally not the git remote), compares it against `getPackageVersion()`, and memoizes a successful result for 10 minutes. [src/electron/updateState.ts](src/electron/updateState.ts) persists `update-state.json` in userData for the 24h background-check gate and the 7-day "Remind Me Later" snooze, following the same `fs`+`try/catch` shape as `legalAcceptance.ts`. The menu-driven and launch-time checks show a native dialog from main; the About modal renders the same `UpdateCheckResult` inline. `app:open-release-page` takes no argument on purpose — the URL comes from the last check, so nothing renderer-supplied reaches `shell.openExternal`.
- **The legal documents are rendered from generated HTML.** [env-scripts/build-legal-docs.mjs](env-scripts/build-legal-docs.mjs) converts `docs/legal/*.md` and the root `LICENSE` into gitignored fragments under `public/legal/`; `docs/` itself is not bundled. The app has no `setWindowOpenHandler`, and its only `shell.openExternal` is the fixed release URL behind `app:open-release-page`, so the script unwraps every link that is not an in-app `#legal-*` tab anchor, and [src/components/LegalDocumentViewer.tsx](src/components/LegalDocumentViewer.tsx) swallows the rest — do not introduce an anchor that would open a bare Electron window. After editing a legal document, re-run `npm run build:legal` and bump `LEGAL_DOC_VERSION` in [src/lib/legal.ts](src/lib/legal.ts) if the change is material, which re-shows the first-run screen for users who already accepted.
